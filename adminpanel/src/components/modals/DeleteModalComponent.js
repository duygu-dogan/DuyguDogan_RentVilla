...
const DeleteModalComponent = ({ delUrl, id, onModalClose, entityName, softDelUrl, isDeleted }) => {
    const modalId = `deleteModal${id}`;
    const handleHardDelete = (e) => {
        e.preventDefault();
        axios.delete(`${delUrl}?id=${id}`)
            .then(response => {
                console.log(response);
                onModalClose(`${entityName} deleted successfully.`, "success");
            })
            .catch(error => {
                console.error(error);
                onModalClose(`${entityName} deletion failed.`, "error");
            });
    }
    const handleSoftDelete = (e) => {
        console.log(id)
        e.preventDefault();
        axios.put(`${softDelUrl}?id=${id}`)
            .then(response => {
                console.log(response);
                onModalClose(`${entityName} moved to recycle bin successfully.`, "success");
            })
            .catch(error => {
                console.error(error);
                onModalClose(`${entityName} movement to recycle bin failed.`, "error");
            });
    }
    return (
        <div>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" hidden={isDeleted} onClick={handleSoftDelete} class="btn btn-warning btn-sm" data-bs-dismiss="modal">Recycle Bin</button>
                            </div>
                        </div>
                    </div>
                </div>
